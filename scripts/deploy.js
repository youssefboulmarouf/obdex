...
const [DAI, BAT, REP, ZRX] = ["DAI", "BAT", "REP", "ZRX"].map(tokenName => ethers.utils.formatBytes32String(tokenName));
...
async function main() {
  ...
  const [dex, dai, bat, rep, zrx] = await Promise.all(
    ["Dex", "Dai", "Bat", "Rep", "Zrx"].map(contractName => deploy(contractName))
  );
